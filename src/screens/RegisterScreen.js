...
import { useNavigation, useRoute } from '@react-navigation/native'; // 🛠️ useRoute ইমপোর্ট করা হলো
import { Eye, EyeOff, Mail, Lock, User, UserPlus, ArrowLeft, Share2 } from 'lucide-react-native';

export default function RegisterScreen() {
  const dispatch = useDispatch();
  const navigation = useNavigation(); 
  const route = useRoute(); // 🔗 নেভিগেশন প্যারামস ধরার জন্য
  
  // Redux থেকে স্টেট নেওয়া
  const { loading, error, isAuthenticated } = useSelector((state) => state.auth);
  
  // 🔗 লিংক থেকে পাঠানো referralCode রিসিভ করা (না থাকলে ফ্যালব্যাক খালি স্ট্রিং)
  const incomingReferralCode = route.params?.referralCode || '';
  // যদি লিংক থেকে কোড আসে, তবে ইউজার এটি পরিবর্তন করতে পারবে না (Lock State)
  const isReferralLocked = incomingReferralCode.length > 0;
  ...
  // রেফারেল কোডের স্টেট (লিংক থেকে আসলে অটো ফিল হবে, নয়তো খালি থাকবে)
  ...
  const handleRegister = async () => {
    ...
    // 🎯 ডাটাবেজে পাঠানোর জন্য অবজেক্ট তৈরি (এখানে referral_code যুক্ত করা হলো)
    const userData = {
      full_name: fullName,
      username: username,
      email: email,
      password: password,
      referral_code: referralCode.trim() || null // খালি থাকলে null যাবে
    };
      ...
      >
        ...
        >
          <View style={styles.card}>
            ...
            {/* 🔑 Referral Code Input (নতুন যুক্ত করা হলো) */}
            <Text style={styles.inputLabel}>Referral Code (Optional)</Text>
            <View style={[
              styles.inputWrapper, 
              isReferralLocked && styles.lockedInputWrapper // লকড স্টেটের জন্য ভিজ্যুয়াল ব্যাকগ্রাউন্ড কালার চেঞ্জ
            ]}>
              <Share2 color={isReferralLocked ? "#10b981" : "#64748b"} size={18} style={styles.icon} />
              <TextInput 
                style={[styles.input, isReferralLocked && { color: "#10b981", fontWeight: 'bold' }]} 
                placeholder="Enter referral code" 
                placeholderTextColor="#475569"
                value={referralCode}
                onChangeText={setReferralCode}
                autoCapitalize="characters" // কোড সবসময় বড় হাতের হবে
                // 🎯 যদি লিংক থেকে আসে (isReferralLocked === true) তবে এডিট করা যাবে না, অন্যথায় এডিটেবল থাকবে
                editable={!loading && !isReferralLocked} 
              />
              {isReferralLocked && (
...
const styles = StyleSheet.create({
  ...
  lockedInputWrapper: { backgroundColor: "#06b6d405", borderColor: "#10b981" }, // লকড বক্সের বর্ডার গ্রিন হবে
  icon: { marginRight: 12 },
  input: { flex: 1, color: "#fff", fontSize: 14, fontWeight: "500", paddingVertical: 12 },
  lockedBadge: { color: '#10b981', fontSize: 10, fontWeight: '900', letterSpacing: 1, backgroundColor: '#10b98115', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  eyeIcon: { padding: 4 },
